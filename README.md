...
- **手动搜索兜底**：文件名只有歌名（没有歌手）或格式特殊时——在待人工队列点「✎ 手动搜索」，直接输入正确的歌名与歌手重新搜索即可；文件名完全不可信时可用音频指纹识别。
- **自动化流程**：候选通过「文件名反推校验」即自动写入并永久记忆（按文件哈希），后续扫描自动跳过，避免重复请求；未通过校验的进入人工辅助队列（候选按「通过校验优先 + 排序分」排列），可逐条查看候选、对比试听与时长、勾选字段后合并写入。打分只用于候选排序，不再决定是否命中。
