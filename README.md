...
- **自动化流程**：候选通过「文件名反推校验」即自动写入并永久记忆（按文件哈希），后续扫描自动跳过，避免重复请求；未通过校验的进入人工辅助队列。打分只用于候选排序，不再决定是否命中。
- **单队列 + 状态过滤器**：全部歌曲合并成一个队列，顶部按状态筛选（全部 / 自动写入 / 待刮削前 / 已人工 / 待人工关 / 跳过 / 错误）；每张卡片的状态标签可点击，手动把这首歌指定为任意状态（指定为「已人工」会永久记住）。
